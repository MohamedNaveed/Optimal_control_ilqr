...
Sk = zeros(model.nx,model.nx,horizon+1); Sk(:,:,horizon+1) = (horizon+1)*QT; %CTG - hessian
...
while iter <= maxIte && criteria
    ...
    while forward_flag
        ...
        for i=1:horizon
            ...
            if i < model.q
                ...
                [state_cost, ~, ~] = model.l(x_new(:,i), model.Xg, model.Q, false);

                cost_new = cost_new + state_cost + 0.5*u_new(:,i)'*R*u_new(:,i);
    ...
    for i=horizon:-1:1
        ...
        %calculate lx and lxx
        [~, lx, lxx] = model.l(x_new(:,i), model.Xg, model.Q, true);
        ...
        Kv(:,:,i) = kpreinv*Bt(:,:,i)';
        Ku(:,:,i) = kpreinv*R;
        Sk(:,:,i) = At(:,:,i)'*Sk(:,:,i+1)*(At(:,:,i)-Bt(:,:,i)*K(:,:,i)) + lxx;
